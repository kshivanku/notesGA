      ...
      if(data.length > 0) {
        ...
        for(var i = 0 ; i < data.length ; i++) {
          if(data[i].source == 'Chrome Extension') {
            $('#notes_list').append('<div class = "newEntry"><div class="text_area"><p class = "content">' + data[i].content + '</p><p class = "source">' + data[i].source + ' | ' + data[i].date + ' | <a href="'+ data[i].url +'" target="_blank">' + data[i].url + '</a></p></div><div id = "' + data[i].content + '" class="delete_btn"></div></div>');
          }
